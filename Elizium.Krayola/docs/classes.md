...
This will result in the following being written to the console:
...
The controlling aspect comes at construction time. The client creates and Scribbler/Krayon via factory functions *New-Scribbler/New-Krayon*. (Actually, the use of New-Krayon is optional as the Krayon is defaulted away in New-Scribbler). Depending on the Flags specified on New-Scribbler and the presence of the environment variable *EliziumTest* a *QuietScribbler* could be created instead of a regular Scribbler. The QuietScribbler behaves like its regular counterpart except that it overrides the Flush method so that the Krayon is not invoked, hence no output.
...
In both of the these examples, the Scribbler will just buffer these instructions up inside an internal StringBuilder and only when the Flush method is called, Scribbler passes the buffered content to Krayon which it interprets as a structure string and invokes host writes accordingly. In the former example, the method calls such as *red* and *blue* in this example are called *accelerators*, because as soon as you call that Scribbler method, the content goes straight into the buffer, where as in the latter example, the user has to build up a structured string and then pass it into the scribble method. This is largely a style choice, one is not better than the other.
...
+ If the client for some reason needed to see in the output, what are now the default tokens 'µ', '«' and '»' in a different context and therefore prefer to change these defaults, they can do so by using a custom regular expression and formatters (see blah). If we stick to not directly using these tokens in our structured strings and use the helper methods on the Scribbler (see blah), we can avoid huge refactors if these patterns/formatters have to change at a later date (that is not to say there are any plans to change the defaults in Krayola).
...
Generally, the user should only interact with the Scribbler class, not the Krayon. The code examples in this section include a *Flush* invoke. It is at this point that the content in the Scribbler's buffer is replayed on the Krayon resulting in the output seen in the console.
...
*value* is a plain string fragment, which means it contains no colouring instructions but just core text. The text is eventually written WITHOUT a line feed. This allows colours to be changed at will on the current line. The text string value is buffered **without** a new line.
...
*value* is a plain string fragment, which means it contains no colouring instructions but just core text. The text string value is buffered **with** a new line.
...
+ Pair/PairLn (see the Pair Class)

#### Scribbler.Pair([couplet]$couplet)

*couplet* is a 'couplet' instance representing an optionally 'affirmed' key/value pair. A couplet's affirmed status is determined by the client and indicates if the value should be rendered using the Krayola theme's *AFFIRM-COLOURS*. Buffers the pair couplet **without** a new line.
...
*couplet* is a 'couplet' instance representing an optionally 'affirmed' key/value pair. A couplet's affirmed status is determined by the client and indicates if the value should be rendered using the Krayola theme's *AFFIRM-COLOURS*. Buffers the pair couplet **with** a new line.
...
*couplet* is a 'PSCustomObject' instance which must have *Key* and *Value* properties representing an optionally *affirmed* boolean member. A couplet's affirmed status is determined by the client and indicates if the value should be rendered using the Krayola theme's *AFFIRM-COLOURS*. Buffers the pair object **with** a new line.
...
+ Line/NakedLine (See the Line class)
...
*line* is a 'Line' instance which is a collection of 'couplet's. The Line method has a new line built into it. The Line method is defined in terms of the Krayola Theme. The collection of pairs that comprises the line are 'book-ended' by the tokens defined in the Krayola theme under the 'OPEN' and 'CLOSE' keys. A naked line is one that is rendered without the 'OPEN' and 'CLOSE' tokens. When *message* is present, the text defined in the Krayola theme under key 'MESSAGE-SUFFIX' is displayed immediately after the message and preceding the line's couplets.
...
+ Message/MessageLn/MessageNoSuffix/MessageNoSuffixLn
...
*message* is some text that precedes static text defined in the Krayola theme under key 'MESSAGE-SUFFIX'. Buffers the message **without** a new line.
...
*message* is some text that precedes static text defined in the Krayola theme under key 'MESSAGE-SUFFIX'. Buffers the message **with** a new line.
...
*message* is some text. The *NoSuffix* version displays the line without static text defined in the Krayola theme under key 'MESSAGE-SUFFIX'. Buffers the message **without** a new line.
...
*message* is some text. The *NoSuffix* version displays the line without static text defined in the Krayola theme under key 'MESSAGE-SUFFIX'. Buffers the message **with** a new line.
...
+ fore/back/defaultFore/defaultBack
...
Buffers the foreground colour **without** a new line.
...
Buffers the background colour **without** a new line.
...
Buffers the default foreground colour **without** a new line.
...
Buffers the default background colour **without** a new line.
...
+ Snippets/WithArgSnippet/PairSnippet/LineSnippet

#### Scribbler.Snippets([string[]]$items)

*$items* a collection of string representing Krayon method invokes.
...
+ fore/back/defaultFore/defaultBack/getDefaultFore/getDefaultBack
...
Same as its Pair equivalent except a new line is appended.
